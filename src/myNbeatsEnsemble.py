...
# from gluonts.model.nbeats._estimator import NBEATSEstimator
from myNbeatsEstimator import *

# Relative imports
# from ._network import VALID_LOSS_FUNCTIONS
from gluonts.model.n_beats._network import VALID_LOSS_FUNCTIONS

# None is also a valid parameter
AGGREGATION_METHODS = "median", "mean", "none"


class myNBEATSEnsemblePredictor(Predictor):
    """"
    ...
    def deserialize(
        ...
        return myNBEATSEnsemblePredictor(
    ...
    def predict(
        ...
        if num_samples != 1:
            logging.warning(
                "myNBEATSEnsemblePredictor does not support sampling. "
...
class myNBEATSEnsembleEstimator(Estimator):
    """
    ...
    **kwargs
    ...
    def train(
        self, training_data: Dataset, validation_data: Optional[Dataset] = None, shuffle_buffer_length = None
    ) -> myNBEATSEnsemblePredictor:
        predictors = []
        ...
        for index, estimator in enumerate(self.estimators):
            ...
            predictors.append(estimator.train(training_data, validation_data, shuffle_buffer_length))
